...
class App:
    def __init__(self, root):
        ...
        # タイマー秒数変更のショートカットを追加
        self.root.bind("<Left>", lambda e: self.sc_t.set(self.sc_t.get() - 1))
        self.root.bind("<Right>", lambda e: self.sc_t.set(self.sc_t.get() + 1))
    ...
    def setup_ui(self):
        ...
        tk.Button(r1, text="🔍 SwitchBotボットを探査", command=self.scan, font=f_b, bg="#4CAF50", fg="white", padx=10).pack(side="left", padx=5)
        ...
        self.cb_cam = ttk.Combobox(r1, state="readonly", width=12, font=f_b)
        ...
        self.cv = tk.Canvas(self.canvas_container, width=w, height=h, bg="#FFD54F", highlightthickness=10, highlightbackground="#FF8F00")
        self.cv.pack()
        self.id_i = self.cv.create_image(w//2, h//2, anchor="center")
        self.id_t = self.cv.create_text(w//2, h//2, text="ボットON", font=("Yu Gothic", 48, "bold"), fill="#333333", anchor="center")
        ...
        self.lbl_esc = tk.Label(self.root, text="※ボットONボタンはマウスドラッグで移動可 / [F11]キー:全画面表示切替 / [1][2]キー:動作モード切替 / [←][→]キー:タイマー秒数変更", font=f_b, bg="#f0f0f0", fg="#555")
    ...
    def act(self):
        if self.mode.get() == 1 and not self.is_running:
            self.run_t()

    def ent(self):
        if self.mode.get() == 2 and not self.is_running:
            self.run_t()
    ...
    def run_t(self):
        ...
        self.send_bot_command("hold", self.remaining)
    ...
    def update_timer(self):
        if self.remaining > 0:
            ...
            self.cv.itemconfig(self.id_t, text="実行中 0秒")
            self.root.after(1000, self.fin_t)

    def fin_t(self):
        self.play_sound(600, 150)
        self.cv.itemconfig(self.id_t, text="ボットON", fill="#333333")
        self.cv.config(highlightbackground="#FF8F00", bg="#FFD54F")
        self.is_running = False
